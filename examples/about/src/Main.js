...
class ExampleLoader extends Component {
    ...
    constructor(item) {
        ...
        function initLoader() {
            window.get(`https://alien.js.org/${item.path}public/assets/js/app.js?${Utils.timestamp()}`).then(data => {
                window.eval(`(()=>{${data.replace('new Main;', `Global.EXAMPLE=Stage;Global.STAGE.add(Stage);Global.STAGE.element.insertBefore(Stage.element, Global.STAGE.element.firstChild);Assets.CDN='https://alien.js.org/${item.path}public/';new Main;`)}})();`);
