...
void main() {
    ...
    vec3 uv = texture2D(texture1, vUv).xyz;

    float gt = mod(time * vUv.x * vUv.y, 2.0 * PI);
    ...
    float r = texture2D(texture1, vUv + d1 * bright).x;
    float g = texture2D(texture1, vUv + d2 * bright).y;
    float b = texture2D(texture1, vUv + d3 * bright).z;

    vec3 uvMix = mix(uv, vec3(r, g, b), 0.2);

    vec3 orig = texture2D(texture2, vUv).xyz;
