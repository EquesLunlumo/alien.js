...
void main() {
    if (uFrame < 10) {
        gl_FragColor = texture2D(tMap2, vUv);
        return;
    }

    vec2 texel = 1.0 / uResolution.xy;

    vec3 uv = texture2D(tMap1, vUv).xyz;
    ...
    float r = texture2D(tMap1, vUv + d1 * bright).x;
    float g = texture2D(tMap1, vUv + d2 * bright).y;
    float b = texture2D(tMap1, vUv + d3 * bright).z;

    vec3 uvMix = mix(uv, vec3(r, g, b), 0.2);

    vec3 orig = texture2D(tMap2, vUv).xyz;
